...
index 0000000000000000000000000000000000000000..2074ea5deee550a41bb469f5e34150520ff9669b
--- /dev/null
+++ b/public/app.js
@@ -0,0 +1,831 @@
...
const apiConfigForm = document.querySelector('#api-base-form');
const apiBaseInput = document.querySelector('#api-base-url');
const apiBaseStatus = document.querySelector('#api-base-status');
const apiBaseResetBtn = document.querySelector('#reset-api-base');
...
+
+  if (apiConfigForm) {
+    apiConfigForm.addEventListener('submit', evt => {
+      evt.preventDefault();
+      const rawValue = apiBaseInput.value.trim();
+      const normalized = normalizeApiBase(rawValue);
+      if (rawValue && !normalized) {
+        alert('Ingresá una URL válida con protocolo (ej: https://midominio.com) o dejala vacía para usar este mismo dominio.');
+        return;
+      }
+
...
+
+  if (apiBaseResetBtn) {
+    apiBaseResetBtn.addEventListener('click', evt => {
+      evt.preventDefault();
+      localStorage.removeItem(API_BASE_STORAGE_KEY);
+      apiBaseUrl = '';
+      updateApiBaseUI();
...
+    const endpoint = buildApiUrl('/api/presupuesto-detalle');
...
+    const normalizedConfig = normalizeApiBase(window.APP_CONFIG.apiBaseUrl);
+    if (normalizedConfig) return normalizedConfig;
+  }
+
+  const meta = document.querySelector('meta[name="api-base-url"]');
+  if (meta && typeof meta.content === 'string' && meta.content.trim()) {
+    const normalizedMeta = normalizeApiBase(meta.content.trim());
...
+}
+
+function replaceQueryParam(param) {
...
+  if (apiBaseStatus) {
+    apiBaseStatus.textContent = apiBaseUrl
+      ? `Consultando datos desde ${apiBaseUrl}`
+      : 'Consultando datos desde este mismo dominio';
+  }
+}
