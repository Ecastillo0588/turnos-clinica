...
createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === '/api/presupuesto-detalle') {
      ...
      const resumen =
        `DETALLE sin IDs | ${fechaDesde} → ${fechaHasta} | páginas≤${MAX_PAGES} | ` +
        `recibidas=${fetchResult.totalRecibidas} | pegadas=${fetchResult.rows.length} | t=${fetchResult.elapsed}s`;
    ...
    if (err.details) body.details = String(err.details).slice(0, 800);
...
async function fetchJson(url) {
  ...
  if (!res.ok) {
    ...
    const err = httpError(502, `HTTP ${res.status} al llamar al servicio.`);
    err.details = text;
    throw err;
  }
  const text = await res.text();
  try {
    ...
    throw httpError(502, 'Respuesta inválida del servicio.');
  }
}
...
function parseDateParam(value, fieldName) {
  if (!value) {
    if (fieldName) throw httpError(400, `Falta el parámetro obligatorio ${fieldName}.`);
    return null;
  }
...
function applyRowFilters(rows, filters) {
  ...
  if (!vendedor && !cliente && !articulo) return rows;
